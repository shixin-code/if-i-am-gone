...
- `Uptime Kuma`：适合自建监控。可以监控 HTTP 端点、TCP 端口、Docker 容器状态，也能转发到 Telegram、邮件或其他通知渠道。
...
2. 查看服务状态：`docker compose ps` 或 `systemctl status ifgone`。
3. 查看日志：`data/state/app.log` 或容器日志。
